...
getWeeklyInsights = async (link, repos) => {
    ...
    $('.text-gray').each(function (i, elem) {
        if($(elem).find('strong')['0'] != undefined){
            commitsToMaster = $(elem).find('strong').find('span')['0'].children[0].data
    ...
    $('.Box-row').each(function (i, elem) {
        if($(elem).find('ul')[0] != undefined){
            ...
            const mergePR = $(elem).find('ul').find('svg')['0'].next.data.trim('\n', ' ')
            const mergeOpenPR = $(elem).find('ul').find('svg')['1'].next.data.trim('\n', ' ')
            const newIssues =  $(elem).find('ul').find('svg')['2'].next.data.trim('\n', ' ')
            const closedIssues = $(elem).find('ul').find('svg')['3'].next.data.trim('\n', ' ')
    ...
    fs.writeFile("repo-reports.json", jsonContent, 'utf8', function (err) {
      if (err) {
  ...
  var links = [
      ...
      'https://github.com/grey-software/LinkedInFocus/pulse',
